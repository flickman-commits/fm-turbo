  ...
  return (
    <div className="relative inline-flex items-center gap-2 w-full sm:w-auto justify-center sm:justify-start">
      <button
        onClick={handleClick}
        className={cn(
          "inline-flex items-center justify-center h-[40px] px-6 py-2 text-sm font-medium text-[#E0CFC0] bg-[#3D0C11] hover:bg-[#3D0C11]/90 rounded-full transition-colors w-full sm:w-auto",
          className
        )}
        {...props}
      >
      ...
      >
        <InfoIcon className="h-4 w-4 text-[#3D0C11] cursor-help sm:cursor-default flex-shrink-0" />
        {showTooltip && (
          <div 
            ref={tooltipRef}
            className={cn(
              "absolute left-1/2 transform -translate-x-1/2 w-64 p-2 text-xs text-[#E0CFC0] bg-[#3D0C11] rounded-md shadow-lg z-50",
          ...
          >
            ...
            <div 
              className={cn(
                "absolute left-1/2 transform -translate-x-1/2 w-2 h-2 bg-[#3D0C11]",
